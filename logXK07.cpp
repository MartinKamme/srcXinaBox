...
{
    global.watchdog();
    bool result = !SD.begin(CS11_SD_CS_PIN);
    result ? OD01.println("1.initSD T") : OD01.println("1.initSD F");
    if (result)
    {
        ui.sdFail();
        OD01.println("2.SD not init");

        SD_CARD_INITIALIZED = false;
        OD01.println("[SD CARD] Card failed, or not present");
        writeInfo("[SD CARD] Card failed or not present", NORMAL);
    }
    else
    {
        OD01.println("3.SD Initied");
        //ui.sdPass();
        OD01.println("4.UI.SD not issue");
        global.watchdog();
